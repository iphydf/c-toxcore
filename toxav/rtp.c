                       ...
                       const struct RTPHeaderV3 *header_v3, uint8_t is_multipart)
{
    ...
    if (wkbl->work_buffer[0].frame_type == video_frame_type_KEYFRAME && is_keyframe == video_frame_type_NORMALFRAME
            && wkbl->work_buffer[0].received_len < wkbl->work_buffer[0].data_len
            && wkbl->work_buffer[0].timestamp + VIDEO_KEEP_KEYFRAME_IN_BUFFER_FOR_MS > net_ntohl(header_v3->timestamp)) {
