...
static State_Load_Status state_load_callback(void *_Nonnull outer, const uint8_t *_Nonnull data, uint32_t length, uint16_t type)
{
    ...
    if (messenger_load_state_section(tox->m, data, length, type, &status)
            || conferences_load_state_section((Group_Chats * _Nonnull)tox->m->conferences_object, data, length, type, &status)) {
        return status;
    }
...
static Tox *tox_new_system(const struct Tox_Options *_Nullable options, Tox_Err_New *_Nullable error, const Tox_System *_Nullable sys)
{
    ...
    const Random *const rng = (const Random * _Nonnull)sys->rng;
    const Network *const ns = (const Network * _Nonnull)sys->ns;
    const Memory *const mem = (const Memory * _Nonnull)sys->mem;
    ...
    g_callback_group_invite((Group_Chats * _Nonnull)tox->m->conferences_object, tox_conference_invite_handler);
    g_callback_group_connected((Group_Chats * _Nonnull)tox->m->conferences_object, tox_conference_connected_handler);
    g_callback_group_message((Group_Chats * _Nonnull)tox->m->conferences_object, tox_conference_message_handler);
    g_callback_group_title((Group_Chats * _Nonnull)tox->m->conferences_object, tox_conference_title_handler);
    g_callback_peer_name((Group_Chats * _Nonnull)tox->m->conferences_object, tox_conference_peer_name_handler);
    g_callback_peer_list_changed((Group_Chats * _Nonnull)tox->m->conferences_object, tox_conference_peer_list_changed_handler);
...
void tox_kill(Tox *tox)
{
    ...
    kill_groupchats(tox->m->conferences_object);
    kill_messenger(tox->m);
    mono_time_free((const Memory * _Nonnull)tox->sys.mem, tox->mono_time);
    tox_unlock(tox);

    if (tox->mutex != nullptr) {
        pthread_mutex_destroy(tox->mutex);
        mem_delete((const Memory * _Nonnull)tox->sys.mem, tox->mutex);
    }

    mem_delete((const Memory * _Nonnull)tox->sys.mem, tox);
}
...
size_t tox_get_savedata_size(const Tox *tox)
{
    ...
    const size_t ret = 2 * sizeof(uint32_t)
                       ...
                       + conferences_size((const Group_Chats * _Nonnull)tox->m->conferences_object)
                       + end_size();
    tox_unlock(tox);
    return ret;
}
...
void tox_get_savedata(const Tox *tox, uint8_t *savedata)
{
    ...
    savedata = conferences_save((const Group_Chats * _Nonnull)tox->m->conferences_object, savedata);
    end_save(savedata);
                                      ...
                                      IP_Port *_Nonnull *root, Tox_Err_Bootstrap *_Nullable error)
{
    ...
    const int32_t count = net_getipport((const Network * _Nonnull)tox->sys.ns, (const Memory * _Nonnull)tox->sys.mem, (const char *_Nonnull)host, root, TOX_SOCK_DGRAM, tox->m->options.dns_enabled);

    if (count < 1) {
        LOGGER_DEBUG(tox->m->log, "could not resolve bootstrap node '%s'", (const char *_Nonnull)host);
...
void tox_iterate(Tox *tox, void *user_data)
{
    ...
    do_groupchats((Group_Chats * _Nonnull)tox->m->conferences_object, &tox_data);
...
bool tox_self_set_name(Tox *tox, const uint8_t *name, size_t length, Tox_Err_Set_Info *error)
{
    ...
    if (setname(tox->m, name, length) == 0) {
        ...
        send_name_all_groups((const Group_Chats * _Nonnull)tox->m->conferences_object);
                       ...
                       const uint8_t *filename, size_t filename_length, Tox_Err_File_Send *error)
{
    ...
    if (file_id == nullptr) {
        ...
        new_symmetric_key((const Random * _Nonnull)tox->sys.rng, f_id);
        file_id = f_id;
    }
...
uint32_t tox_conference_new(Tox *tox, Tox_Err_Conference_New *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = add_groupchat((Group_Chats * _Nonnull)tox->m->conferences_object, (const Random * _Nonnull)tox->sys.rng, GROUPCHAT_TYPE_TEXT);
    tox_unlock(tox);
...
bool tox_conference_delete(Tox *tox, uint32_t conference_number, Tox_Err_Conference_Delete *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const bool ret = del_groupchat((Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, true);
    tox_unlock(tox);
...
uint32_t tox_conference_peer_count(const Tox *tox, uint32_t conference_number, Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_number_peers((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, false);
    tox_unlock(tox);
        ...
        Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peername_size((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, peer_number, false);
    tox_unlock(tox);
                                  ...
                                  Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peername((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, peer_number, name, false);
    tox_unlock(tox);
                                        ...
                                        uint8_t public_key[TOX_PUBLIC_KEY_SIZE], Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peer_pubkey((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, peer_number, public_key, false);
    tox_unlock(tox);
                                        ...
                                        Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peernumber_is_ours((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, peer_number);
    tox_unlock(tox);
        ...
        Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_number_peers((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, true);
    tox_unlock(tox);
        ...
        Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peername_size((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, offline_peer_number, true);
    tox_unlock(tox);
        ...
        Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peername((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, offline_peer_number, name, true);
    tox_unlock(tox);
        ...
        uint8_t public_key[TOX_PUBLIC_KEY_SIZE], Tox_Err_Conference_Peer_Query *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_peer_pubkey((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, offline_peer_number, public_key, true);
    tox_unlock(tox);
        ...
        Tox_Err_Conference_Peer_Query *error)
{
    ...
    tox_lock(tox);
    const int ret = group_frozen_last_active((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, offline_peer_number,
                    &last_active);
    tox_unlock(tox);
                                    ...
                                    Tox_Err_Conference_Set_Max_Offline *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_set_max_frozen((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, max_offline);
    tox_unlock(tox);
                           ...
                           Tox_Err_Conference_Invite *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = invite_friend((const Group_Chats * _Nonnull)tox->m->conferences_object, friend_number, conference_number);
    tox_unlock(tox);
                             ...
                             Tox_Err_Conference_Join *error)
{
    ...
    tox_lock(tox);
    const int ret = join_groupchat((Group_Chats * _Nonnull)tox->m->conferences_object, friend_number, GROUPCHAT_TYPE_TEXT, cookie, length);
    tox_unlock(tox);
                                 ...
                                 size_t length, Tox_Err_Conference_Send_Message *error)
{
    ...
    if (type == TOX_MESSAGE_TYPE_NORMAL) {
        ret = group_message_send((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, message, length);
    } else {
        ret = group_action_send((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, message, length);
    }
...
size_t tox_conference_get_title_size(const Tox *tox, uint32_t conference_number, Tox_Err_Conference_Title *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_title_get_size((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number);
    tox_unlock(tox);
                              ...
                              Tox_Err_Conference_Title *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_title_get((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, title);
    tox_unlock(tox);
                              ...
                              Tox_Err_Conference_Title *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_title_send((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, title, length);
    tox_unlock(tox);
...
size_t tox_conference_get_chatlist_size(const Tox *tox)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const size_t ret = count_chatlist((const Group_Chats * _Nonnull)tox->m->conferences_object);
    tox_unlock(tox);
    return ret;
}
...
void tox_conference_get_chatlist(const Tox *tox, uint32_t *chatlist)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const size_t list_size = count_chatlist((const Group_Chats * _Nonnull)tox->m->conferences_object);
    copy_chatlist((const Group_Chats * _Nonnull)tox->m->conferences_object, chatlist, list_size);
    tox_unlock(tox);
}
        ...
        Tox_Err_Conference_Get_Type *error)
{
    assert(tox != nullptr);
    tox_lock(tox);
    const int ret = group_get_type((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number);
    tox_unlock(tox);
...
bool tox_conference_get_id(const Tox *tox, uint32_t conference_number, uint8_t id[TOX_CONFERENCE_ID_SIZE])
{
    assert(tox != nullptr);
    tox_lock(tox);
    const bool ret = conference_get_id((const Group_Chats * _Nonnull)tox->m->conferences_object, conference_number, id);
    tox_unlock(tox);
    return ret;
}
...
uint32_t tox_conference_by_id(const Tox *tox, const uint8_t id[TOX_CONFERENCE_ID_SIZE], Tox_Err_Conference_By_Id *error)
{
    ...
    tox_lock(tox);
    const int32_t ret = conference_by_id((const Group_Chats * _Nonnull)tox->m->conferences_object, id);
    tox_unlock(tox);
