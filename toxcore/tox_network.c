...
{
    Tox_Network *ns = (Tox_Network *)tox_memory_alloc(mem, sizeof(struct Tox_Network));
...
int tox_network_bind(const Tox_Network *ns, Socket sock, const IP_Port *addr)
{
...
int tox_network_connect(const Tox_Network *ns, Socket sock, const IP_Port *addr)
{
...
int tox_network_recvfrom(const Tox_Network *ns, Socket sock, uint8_t *buf, size_t len, IP_Port *addr)
{
...
int tox_network_sendto(const Tox_Network *ns, Socket sock, const uint8_t *buf, size_t len, const IP_Port *addr)
{
...
int tox_network_getaddrinfo(const Tox_Network *ns, const Tox_Memory *mem, const char *address, int family, int protocol, IP_Port **addrs)
{
    return ns->funcs->getaddrinfo_callback(ns->user_data, mem, address, family, protocol, addrs);
}

int tox_network_freeaddrinfo(const Tox_Network *ns, const Tox_Memory *mem, IP_Port *addrs)
{
