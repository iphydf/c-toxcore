 ...
 */
int32 create_extended_keypair(uint8_mut *pk, uint8_mut *sk)
{
...
static uint8_mut *crypto_malloc(size_t bytes)
{
    return (uint8_mut *)malloc(bytes);
}

static void crypto_free(uint8_mut *ptr, size_t bytes)
{
    if (ptr != nullptr) {
...
int32 public_key_cmp(uint8 *pk1, uint8 *pk2)
{
    return crypto_verify_32(pk1, pk2);
}

uint8 random_u08(void)
{
    uint8_mut randnum;
...
uint16 random_u16(void)
{
    uint16_mut randnum;
    random_bytes((uint8_mut *)&randnum, sizeof(randnum));
    return randnum;
}

uint32 random_u32(void)
{
    uint32_mut randnum;
    random_bytes((uint8_mut *)&randnum, sizeof(randnum));
    return randnum;
...
uint32 random_int_range(uint32 upper_bound)
{
    return randombytes_uniform(upper_bound);
}
#endif

bool public_key_valid(uint8 *public_key)
{
 ...
 */
int32 encrypt_precompute(uint8 *public_key, uint8 *secret_key,
                         uint8_mut *shared_key)
{
    return crypto_box_beforenm(shared_key, public_key, secret_key);
}

int32 encrypt_data_symmetric(uint8 *secret_key, uint8 *nonce,
                             uint8 *plain, size_t length, uint8_mut *encrypted)
{
    ...
    uint8_mut *temp_plain = crypto_malloc(size_temp_plain);
    uint8_mut *temp_encrypted = crypto_malloc(size_temp_encrypted);
...
int32 decrypt_data_symmetric(uint8 *secret_key, uint8 *nonce,
                             uint8 *encrypted, size_t length, uint8_mut *plain)
{
...
int32 encrypt_data(uint8 *public_key, uint8 *secret_key, uint8 *nonce,
                   uint8 *plain, size_t length, uint8_mut *encrypted)
{
    ...
    uint8_mut k[crypto_box_BEFORENMBYTES];
...
int32 decrypt_data(uint8 *public_key, uint8 *secret_key, uint8 *nonce,
                   uint8 *encrypted, size_t length, uint8_mut *plain)
{
    if (!public_key || !secret_key) {
        return -1;
...
void increment_nonce(uint8_mut *nonce)
{
     ...
     */
    uint32_mut i = crypto_box_NONCEBYTES;
    uint_fast16_t carry = 1U;

    for (; i != 0; --i) {
        carry += (uint_fast16_t)nonce[i - 1];
        nonce[i - 1] = (uint8)carry;
        carry >>= 8;
    }
}

static uint32 host_to_network(uint32 x)
{
...
void increment_nonce_number(uint8_mut *nonce, uint32 host_order_num)
{
    /* NOTE don't use breaks inside this loop
     * In particular, make sure, as far as possible,
     * that loop bounds and their potential underflow or overflow
     * are independent of user-controlled input (you may have heard of the Heartbleed bug).
     */
    uint32 big_endian_num = host_to_network(host_order_num);
    uint8 *const num_vec = (uint8 *)&big_endian_num;
    uint8_mut num_as_nonce[crypto_box_NONCEBYTES] = {0};
...
void random_nonce(uint8_mut *nonce)
{
    random_bytes(nonce, crypto_box_NONCEBYTES);
}

/* Fill a key CRYPTO_SYMMETRIC_KEY_SIZE big with random bytes */
void new_symmetric_key(uint8_mut *key)
{
    random_bytes(key, CRYPTO_SYMMETRIC_KEY_SIZE);
}

int32 crypto_new_keypair(uint8_mut *public_key, uint8_mut *secret_key)
{
    return crypto_box_keypair(public_key, secret_key);
}

void crypto_derive_public_key(uint8_mut *public_key, uint8 *secret_key)
{
    crypto_scalarmult_curve25519_base(public_key, secret_key);
}

void crypto_sha256(uint8_mut *hash, uint8 *data, size_t length)
{
    crypto_hash_sha256(hash, data, length);
}

void crypto_sha512(uint8_mut *hash, uint8 *data, size_t length)
{
    crypto_hash_sha512(hash, data, length);
}

void random_bytes(uint8_mut *data, size_t length)
{
