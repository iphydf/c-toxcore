...
non_null()
static bool tox_event_type_unpack(Tox_Event_Type *type, Bin_Unpack *bu)
{
    uint8_t u08;
    if (!bin_unpack_u08(bu, &u08)) {
        return false;
    }

    if (u08 <= TOX_EVENT_INVALID || u08 > TOX_EVENT_MAX) {
        return false;
    }

    *type = (Tox_Event_Type)u08;
    return true;
}
...
bool tox_event_unpack_into(Tox_Event *event, Bin_Unpack *bu, const Memory *mem)
{
    ...
    Tox_Event_Type type;
    if (!tox_event_type_unpack(&type, bu)) {
        return false;
    }
