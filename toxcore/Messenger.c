                         ...
                         Messenger_Options *options, Messenger_Error *error)
{
    ...
    Net_Crypto *net_crypto = new_net_crypto(m->log, m->mem, m->rng, m->ns, m->mono_time, m->net, m->dht, &options->proxy_info, m->tcp_np);
    ...
    if (options->dht_announcements_enabled) {
        ...
        if (m->forwarding != nullptr) {
            m->announce = new_announcements(m->log, m->mem, m->rng, m->mono_time, (Forwarding *_Nonnull)m->forwarding, m->dht, m->net);
        }
    }
