...
typedef int tox_network_bind_cb(void *self, Socket sock, const IP_Port *addr);
typedef int tox_network_listen_cb(void *self, Socket sock, int backlog);
typedef int tox_network_connect_cb(void *self, Socket sock, const IP_Port *addr);
...
typedef int tox_network_recvfrom_cb(void *self, Socket sock, uint8_t *buf, size_t len, IP_Port *addr);
typedef int tox_network_send_cb(void *self, Socket sock, const uint8_t *buf, size_t len);
typedef int tox_network_sendto_cb(void *self, Socket sock, const uint8_t *buf, size_t len, const IP_Port *addr);
...
typedef int tox_network_getaddrinfo_cb(void *obj, const Tox_Memory *mem, const char *address, int family, int protocol, IP_Port **addrs);
typedef int tox_network_freeaddrinfo_cb(void *obj, const Tox_Memory *mem, IP_Port *addrs);
