...
int mod_list_unpack(Moderation *moderation, const uint8_t *data, uint16_t length, uint16_t num_mods)
{
    ...
    uint8_t **owner tmp_list = (uint8_t **owner)calloc(num_mods, sizeof(uint8_t *));
    ...
    for (uint16_t i = 0; i < num_mods; ++i) {
        uint8_t *owner entry = (uint8_t *owner)malloc(MOD_LIST_ENTRY_SIZE);
...
bool mod_list_remove_index(Moderation *moderation, uint16_t index)
{
    ...
    uint8_t **owner tmp_list = (uint8_t **owner)realloc(moderation->mod_list, moderation->num_mods * sizeof(uint8_t *));
...
bool mod_list_add_entry(Moderation *moderation, const uint8_t *mod_data)
{
    ...
    uint8_t **owner tmp_list = (uint8_t **owner)realloc(moderation->mod_list, (moderation->num_mods + 1) * sizeof(uint8_t *));
 ...
 */
non_null()
static Mod_Sanction *owner sanctions_list_copy(const Mod_Sanction *sanctions, uint16_t num_sanctions)
{
    Mod_Sanction *owner copy = (Mod_Sanction *owner)calloc(num_sanctions, sizeof(Mod_Sanction));
...
static bool sanctions_list_remove_index(Moderation *moderation, uint16_t index, const Mod_Sanction_Creds *creds)
{
    ...
    Mod_Sanction *owner sanctions_copy = sanctions_list_copy(moderation->sanctions, moderation->num_sanctions);
    ...
    Mod_Sanction *owner new_list = (Mod_Sanction *owner)realloc(sanctions_copy, new_num * sizeof(Mod_Sanction));
...
bool sanctions_list_add_entry(Moderation *moderation, const Mod_Sanction *sanction, const Mod_Sanction_Creds *creds)
{
    ...
    Mod_Sanction *owner new_list = (Mod_Sanction *owner)realloc(sanctions_copy, (index + 1) * sizeof(Mod_Sanction));
