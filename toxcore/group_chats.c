                               ...
                               uint8_t *packet_type, const uint8_t *packet, uint16_t length)
{
    ...
    uint8_t *owner plain = (uint8_t *owner)malloc(length);
    ...
    uint8_t gp_packet_type, Net_Packet_Type net_packet_type)
{
    ...
    uint8_t *owner plain = (uint8_t *owner)malloc(packet_size);
...
static bool sync_response_send_peers(GC_Chat *chat, GC_Connection *gconn, uint32_t peer_number, bool first_sync)
{
    ...
    uint8_t *owner response = (uint8_t *owner)malloc(MAX_GC_PACKET_CHUNK_SIZE);
...
{
    GC_Peer *owner self = (GC_Peer *owner)calloc(1, sizeof(GC_Peer));
    ...
    uint8_t *owner data = (uint8_t *owner)malloc(data_size);
                                        ...
                                        const uint8_t *data, uint16_t length, void *userdata)
{
    ...
    GC_Peer *owner peer_info = (GC_Peer *owner)calloc(1, sizeof(GC_Peer));
                                    ...
                                    void *userdata)
{
    ...
    Mod_Sanction *owner sanctions = (Mod_Sanction *owner)calloc(num_sanctions, sizeof(Mod_Sanction));
...
static int make_gc_mod_list_packet(const GC_Chat *chat, uint8_t *data, uint32_t maxlen, uint16_t mod_list_size)
{
    ...
    if (mod_list_size > 0) {
        uint8_t *owner packed_mod_list = (uint8_t *owner)malloc(mod_list_size);
...
static bool send_peer_mod_list(const GC_Chat *chat, GC_Connection *gconn)
{
    ...
    const uint16_t length = sizeof(uint16_t) + mod_list_size;
    uint8_t *owner packet = (uint8_t *owner)malloc(length);
...
static bool send_peer_topic(const GC_Chat *chat, GC_Connection *gconn)
{
    ...
    uint8_t *owner packet = (uint8_t *owner)malloc(packet_buf_size);
...
int gc_set_topic(GC_Chat *chat, const uint8_t *topic, uint16_t length)
{
    ...
    uint8_t *owner packed_topic = (uint8_t *owner)malloc(packet_buf_size);
...
int gc_founder_set_password(GC_Chat *chat, const uint8_t *password, uint16_t password_length)
{
    ...
    uint8_t *owner oldpasswd = memdup(chat->shared_state.password, oldlen);
...
static bool send_gc_set_mod(const GC_Chat *chat, const GC_Connection *gconn, bool add_mod)
{
    ...
    uint8_t *owner data = (uint8_t *owner)malloc(length);
                                 ...
                                 const uint8_t *sanction_data, uint16_t length, bool add_obs)
{
    ...
    uint8_t *owner packet = (uint8_t *owner)malloc(packet_len);
...
int gc_send_message(const GC_Chat *chat, const uint8_t *message, uint16_t length, uint8_t type, uint32_t *message_id)
{
    ...
    uint8_t *owner message_raw = (uint8_t *owner)malloc(length_raw);
                            ...
                            uint16_t length, uint32_t *message_id)
{
    ...
    uint8_t *owner message_with_type = (uint8_t *owner)malloc(raw_length);
    ...
    uint8_t *owner packet = (uint8_t *owner)malloc(raw_length + GC_BROADCAST_ENC_HEADER_SIZE);
                                      ...
                                      const uint8_t *packet, uint16_t length, bool direct_conn, void *userdata)
{
    ...
    uint8_t *owner data = (uint8_t *owner)malloc(data_buf_size);
...
static bool peer_delete(const GC_Session *c, GC_Chat *chat, uint32_t peer_number, void *userdata)
{
    ...
    GC_Peer *owner tmp_group = (GC_Peer *owner)realloc(chat->group, chat->numpeers * sizeof(GC_Peer));
...
int peer_add(GC_Chat *chat, const IP_Port *ipp, const uint8_t *public_key)
{
    ...
    GC_Message_Array_Entry *owner send = (GC_Message_Array_Entry *owner)calloc(GCC_BUFFER_SIZE, sizeof(GC_Message_Array_Entry));
    GC_Message_Array_Entry *owner recv = (GC_Message_Array_Entry *owner)calloc(GCC_BUFFER_SIZE, sizeof(GC_Message_Array_Entry));
    ...
    GC_Peer *owner tmp_group = (GC_Peer *owner)realloc(chat->group, (chat->numpeers + 1) * sizeof(GC_Peer));
...
static bool ping_peer(const GC_Chat *chat, const GC_Connection *gconn)
{
    ...
    uint8_t *owner data = (uint8_t *owner)malloc(buf_size);
...
static bool realloc_groupchats(GC_Session *c, uint32_t n)
{
    ...
    GC_Chat *owner temp = (GC_Chat *owner)realloc(c->chats, n * sizeof(GC_Chat));
...
static void add_tcp_relays_to_chat(const GC_Session *c, GC_Chat *chat)
{
    ...
    Node_format *owner tcp_relays = (Node_format *owner)calloc(num_relays, sizeof(Node_format));
                     ...
                     gc_send_group_invite_packet_cb *callback)
{
    ...
    uint8_t *owner packet = (uint8_t *owner)malloc(2 + CHAT_ID_SIZE + ENC_PUBLIC_KEY_SIZE + group_name_length);
        ...
        const uint8_t *data, uint16_t length)
{
    ...
    uint8_t *owner packet = (uint8_t *owner)malloc(packet_length);
...
GC_Session *new_dht_groupchats(Messenger *m)
{
    ...
    GC_Session *owner c = (GC_Session *owner)calloc(1, sizeof(GC_Session));
