...
static int set_gc_password_local(GC_Chat *chat, const uint8_t *passwd, uint16_t length)
{
    ...
    if (!passwd || length == 0) {
        ...
        memset(chat->shared_state.password, 0, MAX_GC_PASSWD_SIZE);
    } else {
...
static uint16_t pack_gc_shared_state(uint8_t *data, uint16_t length, const GC_SharedState *shared_state)
{
    ...
    packed_len += sizeof(uint16_t);
    memcpy(data + packed_len, shared_state->password, MAX_GC_PASSWD_SIZE);
...
static uint16_t unpack_gc_shared_state(GC_SharedState *shared_state, const uint8_t *data, uint16_t length)
{
    ...
    len_processed += sizeof(uint16_t);
    memcpy(shared_state->password, data + len_processed, MAX_GC_PASSWD_SIZE);
...
static int send_gc_sync_request(GC_Chat *chat, GC_Connection *gconn, uint32_t num_peers)
{
    ...
    memcpy(data + HASH_ID_BYTES + sizeof(uint32_t), chat->shared_state.password, MAX_GC_PASSWD_SIZE);
                                   ...
                                   const uint8_t *data, uint32_t length)
{
    ...
    if (num_peers > MAX_GC_NUM_PEERS) {  // TODO: remove
    ...
    if (num_peers) {
        ...
        for (i = 0; i < num_peers; i++) {
            ...
            GC_Connection *peer_conn = gcc_get_connection(chat, peer_number);
            if (!peer_conn) {
                continue;
            }

            for (j = 0; j < announces->tcp_relays_count; j++) {
                add_tcp_relay_connection(chat->tcp_conn, peer_conn->tcp_connection_num,
            ...
            if (curr_announce->ip_port_is_set && !is_tcp_mode) {
                send_gc_handshake_packet(chat, peer_number, GH_REQUEST, HS_PEER_INFO_EXCHANGE, chat->join_type);
                                  ...
                                  uint32_t length)
{
    ...
    if (chat->shared_state.password_length > 0) {
        ...
        memcpy(password, data + sizeof(uint32_t), MAX_GC_PASSWD_SIZE);
...
static int send_gc_invite_request(GC_Chat *chat, GC_Connection *gconn)
{
    ...
    memcpy(data + length, chat->shared_state.password, MAX_GC_PASSWD_SIZE);
...
static int send_self_to_peer(const GC_Session *c, GC_Chat *chat, GC_Connection *gconn)
{
    ...
    uint8_t data[MAX_GC_PACKET_SIZE];
    net_pack_u32(data, chat->self_public_key_hash);
    memcpy(data + HASH_ID_BYTES, chat->shared_state.password, MAX_GC_PASSWD_SIZE);
                                        ...
                                        const uint8_t *data, uint32_t length)
{
    ...
    if (chat->shared_state.password_length > 0) {
        ...
        if (memcmp(chat->shared_state.password, passwd, chat->shared_state.password_length) != 0) {
            return -1;
        }
    }
...
static int validate_gc_shared_state(const GC_SharedState *state)
{
    ...
    if (state->password_length > MAX_GC_PASSWD_SIZE) {
        return -1;
    }
                                       ...
                                       const uint8_t *data, uint32_t length)
{
    ...
    int nodes_count = unpack_nodes(node, MAX_ANNOUNCED_TCP_RELAYS, NULL, data + processed, length - processed, 1);
                                      ...
                                      bool direct_conn)
{
    ...
    if (packet_type != GP_HS_RESPONSE_ACK && !gconn->handshaked) { // TODO: fix race condition between ack and invite request
...
int gc_group_load(GC_Session *c, struct Saved_Group *save)
{
    ...
    memcpy(chat->shared_state.password, save->passwd, MAX_GC_PASSWD_SIZE);
 ...
 */
int gc_invite_friend(GC_Session *c, GC_Chat *chat, int32_t friend_number, gc_send_group_invite_packet_cb *send_group_invite_packet)
{
...
static void copy_ip_port(Messenger *m, int friend_number, GC_Connection *gconn)
{
    ...
    memcpy(&gconn->addr.ip_port, &connection->dht_ip_port, sizeof(IP_Port));
}
                                      ...
                                      uint32_t length)
{
    ...
    if (num_nodes <= 0 && is_tcp_only) {
        return -1;
    }

    copy_ip_port(c->messenger, friend_number, gconn);
...
int add_peers_from_announces(const GC_Session *gc_session, GC_Chat *chat, GC_Peer_Announce *announces, uint8_t gc_announces_count)
{
    ...
    for (i = 0; i < gc_announces_count; i++) {
        ...
        IP_Port *ip_port = curr_announce->base_announce.ip_port_is_set ? &curr_announce->base_announce.ip_port : NULL;
        ...
        for (j = 0; j < curr_announce->base_announce.tcp_relays_count; j++) {
            int add_tcp_result = add_tcp_relay_connection(chat->tcp_conn, gconn->tcp_connection_num,
        ...
        if (curr_announce->base_announce.ip_port_is_set && !curr_announce->base_announce.tcp_relays_count) {
            send_gc_handshake_packet(chat, peer_number, GH_REQUEST, HS_INVITE_REQUEST, chat->join_type);
            // TODO: proper check how to send handshake
        } else {
            gconn->is_oob_handshake = true;
            gconn->is_pending_handshake_response = false;
            gconn->pending_handshake_type = HS_INVITE_REQUEST;
