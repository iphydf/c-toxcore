...
static void group_callback_update_addresses(GC_Announce *announce, void (*function)(GC_Announce *, const uint8_t *,
        void *),
        void *object)
{
                                   ...
                                   uint32_t length)
{
    ...
    if (num_peers) {
        ...
        uint32_t *peer_ids = (uint32_t *)malloc(sizeof(uint32_t) * num_peers);

        const uint8_t *chat_pk = data + length - public_keys_size;
        ...
        for (int index = 0; index < added_peers_index; index++) {
...
int gc_set_topic(GC_Chat *chat, const uint8_t *topic, uint16_t length)
{
    ...
    if (crypto_sign_detached(chat->topic_sig, nullptr, packed_topic, packed_len, SIG_SK(chat->self_secret_key)) == -1) {
        goto on_error;
    }
