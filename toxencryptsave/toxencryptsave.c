                          ...
                          uint8_t ciphertext[], Tox_Err_Encryption *error)
{
    ...
    /* now encrypt */
    const int32_t encrypted_len = encrypt_data_symmetric((const Memory * _Nonnull)os_memory(), key->key, nonce, plaintext, plaintext_len, ciphertext);
                          ...
                          uint8_t plaintext[], Tox_Err_Decryption *error)
{
    ...
    const int32_t decrypted_len = decrypt_data_symmetric((const Memory * _Nonnull)os_memory(), key->key, nonce, ciphertext, decrypt_length + crypto_box_MACBYTES, plaintext);
